...
class LineSearch:
    def __init__(self, xtol=1e-14):
        self.xtol = xtol
        self.task = 'START'
        self.isave = torch.zeros((2,), dtype=torch.int32)
        self.dsave = torch.zeros((13,), dtype=torch.float64)
        self.fc = 0
        self.gc = 0
        self.case = 0
        self.old_stp = 0
    ...
    def _line_search(self, func, myfprime, xk, pk, gfk, old_fval, old_old_fval,
                     ...
                     stpmax=50., stpmin=1e-8, args=(), cur_step=0):
        self.stpmin = stpmin
        self.pk = pk
        self.stpmax = stpmax
        self.xtrapl = xtrapl
        ...
        self.dim = len(pk)
        self.gms = torch.sqrt(torch.tensor(self.dim, dtype=torch.float64)) * maxstep
        alpha1 = 1.
        ...
        while True:
            ...
            # print(f'&&& isave: {self.isave}')
            # print(f'&&& dsave: {self.dsave}')
...
#       quadratic steps is taken.
        if fp > fx:  # case1
            ...
            gamma = s * torch.sqrt((theta / s) ** 2. - (gx / s) * (gp / s))
            if stp < stx:
                gamma = -gamma
            p = (gamma - gx) + theta
            q = ((gamma - gx) + gamma) + gp
...
#           secant step.
            ...
            theta = 3. * (fx - fp) / (stp - stx) + gx + gp
            s = max(abs(theta), abs(gx), abs(gp))
            gamma = s * torch.sqrt(pymax(0., (theta / s) ** 2 - (gx / s) * (gp / s)))
            if stp > stx:
                gamma = -gamma
            p = (gamma - gp) + theta
            q = (gamma + (gx - gp)) + gamma
...
#       otherwise the cubic step is taken.
            ...
            if self.bracket:
                ...
                gamma = s * torch.sqrt((theta / s) ** 2 - (gy / s) * (gp / s))
                if stp > sty:
                    gamma = -gamma
                p = (gamma - gp) + theta
                q = ((gamma - gp) + gamma) + gy
...
#       Compute the new step.
    ...
    def determine_step(self, stp):
        dr = stp - self.old_stp
        x = torch.reshape(self.pk, (-1, 3))
        # print(f'^^^ x: {x}')
        # print(f'^^^ dr: {dr}')
        steplengths = torch.sqrt(((dr * x)**2).sum(1))
